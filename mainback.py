...
def get_wallet_balance(mnemonic: list[str], toncenter_api_key: str = "") -> float:
    # Получаем ключи
    ...
    if not data.get("ok"):
        raise Exception(f"Ошибка от toncenter: {data}")
...
def get_solana_balance(mnemonic: str, rpc_url: str = "https://api.mainnet-beta.solana.com") -> float:
    # Генерация seed из мнемоники
    seed_bytes = Bip39SeedGenerator(mnemonic).Generate()

    # Генерация ключей для Solana (BIP44 путь)
    ...
    if "result" in balance_response:
        ...
        return lamports / 1_000_000_000  # Перевод из lamports в SOL
    else:
        raise Exception(f"RPC error: {balance_response}")

# Пример использования
if __name__ == "__main__":
    mnemonic = input("Введите seed-фразу (12 или 24 слова): ").strip()
    try:
        balance = get_solana_balance(mnemonic)
        print(f"Баланс кошелька: {balance:.9f} SOL")
...
while True:
    try:
        print(get_wallet_balance(mnemonic=random.choices(WORDLIST, k=24), toncenter_api_key="016049298b5191241b0e95d8457d20536011fbb87c53e18bb693193e0646f763"))
    except:
        continue